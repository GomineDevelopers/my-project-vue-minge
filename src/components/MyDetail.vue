<template>
  <div class="news-detail-container" v-show="isLoadFinish">
    <div class="news-detail-top">
      ...
      <el-row class="news-detail-top-row">
        <el-col :span="24">
          <div class="news-detail-subtitle">
            ...
            <el-row>
              <el-col :span="12">
                <div class="sourceDiv" v-text="source_c"></div>
              </el-col>
              <el-col :span="12">
                <div v-text="article_time_c"></div>
              </el-col>
            </el-row>
          </div>
        </el-col>
      ...
      <el-row class="news-detail-top-row">
        <el-col :span="24">
          <div class="news-detail-top-article" v-html="content" id="newsContent"></div>
          <img v-gallery :src="imgUrl" id="currentImg" hidden>
        </el-col>
      </el-row>
    </div>
    ...
    computed: {
      article_time_c: function () {
        return this.article_time
          ? '发布日期：' + this.$commonTools.formatDate(this.article_time)
          : ''
      },
      source_c: function () {
    ...
    watch: {
      ...
      isLoadFinish() {
        ...
        self.$nextTick(function () {
          for (let i = 0; i < document.getElementById("newsContent").getElementsByTagName("img").length; i++) {
            document.getElementById("newsContent").getElementsByTagName("img")[i].addEventListener("click", function () {
              self.imgUrl = this.src;
              self.$nextTick(function () {
                document.getElementById("currentImg").click()
              })
            });
          }
        })
      ...
      getDetailData: function () {
        ...
        vm.axios(vm.$commonTools.g_restUrl, {
          ...
          .then(function (response) {
            ...
            if (
              response.data.result.data.ccateid == 37 ||
              response.data.result.data.ccateid == 40 ||
              response.data.result.data.ccateid == 56 ||
              response.data.result.data.ccateid == 59
            ) {
              vm.is_showCommon = false
            }

            vm.common_list.forEach(function (element, index, array) {
              element.create_time = vm.$commonTools.formatHour(
                element.create_time
              )
            })
