<template>
  <div class="quick_validate_bg">
    ...
    <div class="register-container">
      ...
      <el-row>
        <el-col :span="24"><span class="register-spanblock"><span class="register-necessary">*</span>工作经历</span>
        </el-col>
      </el-row>
      <!--<el-row>
        <el-col :span="24">
      ...
      <el-row>
        <el-col :span="24">
          <div @click="addExperience" class="experience">+添加工作经历</div>
        </el-col>
      </el-row>

      <el-row v-for="(item,index) in WorkExperiences" :key="index">
        <div @click="goDetail(index)">
          <el-input class="inputText" v-model="WorkExperiences[index]" clearable readonly></el-input>
        </div>
      </el-row>
      <el-row>
      ...
      <el-row>
        <div @click="addApplication">
          <el-input class="inputText " type="textarea" :rows="6" v-model="applycation" readonly></el-input>
        </div>
      </el-row>
  ...
  export default {
    ...
    methods: {
      getExitData() {
        ...
        if (vm.$commonTools.getCookie("cookieData")) {
          let cookieData = JSON.parse(vm.$commonTools.getCookie("cookieData"));
          vm.registerName = decodeURI(cookieData.realname);
          vm.registerBirthday = cookieData.birth;
          vm.ethnicGroup = decodeURI(cookieData.ethnicgroup);
          vm.distmix = decodeURI(cookieData.origin);
          vm.degreeValue = decodeURI(cookieData.education);
      ...
      setApplyCookies: function () {
        ...
        postData.realname =  encodeURI(vm.registerName);
        postData.sex = vm.$children[5].$children[0].$children[0].defaultValue;
        postData.birth = vm.registerBirthday;
        postData.ethnicgroup = encodeURI(vm.ethnicGroup);
        postData.origin = encodeURI(vm.distmix);
        postData.education = encodeURI(vm.degreeValue);
